...
namespace Battleship
{
    public class Battleship : Ship
    {
        ...
        public void PlaceShip(Destroyer destroyer, Submarine submarine, AircraftCarrier aircraftCarrier)
        {
            locationSet = false;
            ...
            if (!GoodCoordinates(x1, y1, x2, y2, destroyer, submarine, aircraftCarrier))
            {
                Console.WriteLine($"I'm sorry, it doesn't seem like those coordinates work for the {name}.  Let's try again.");
                location = ClearLocation(location);
                PlaceShip(destroyer, submarine, aircraftCarrier);
            }
            if (!locationSet)
            {
                  SetCoordinates();
        ...
        public bool GoodCoordinates(string stringx1, string stringy1, string stringx2, string stringy2, Destroyer destroyer, Submarine submarine, AircraftCarrier aircraftCarrier)
        {
            ...
            if (CoordinateIsInt(stringx1) && CoordinateIsInt(stringx2) && CoordinateIsInt(stringy1) && CoordinateIsInt(stringy2))
            {
                ...
                if (ValidBoardCoordinate(x1, y1) && ValidBoardCoordinate(x2, y2))
                {
                    if (ValidShipOnBoard(x1, y1, x2, y2))
                    {
                        ...
                        if (NoOtherShipConflict(destroyer, submarine, aircraftCarrier))
                        {
